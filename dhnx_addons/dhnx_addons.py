...
def pp_plot_results(pipes, consumers, producers, forks, forks_p_min,
    ...
    if show_plot or save_plot:
        ...
        if direction == 'circular':
            ...
            if save_plot:
                custom_plot_save(os.path.join(save_path, 'plots', 'Pressure vs Distance'))
            if show_plot:
                plt.show()
            else:
                plt.close()
    ...
    if save_path is not None:
        ...
        save_gis_generic(producers, 'pandapipes_producers', path=save_path,
                         ext=save_gis_ext)
        save_gis_generic(gdf_shortest, 'pandapipes_shortest', path=save_path,
                         ext=save_gis_ext)
        if direction == 'circular':
            save_gis_generic(gdf_shortest_f, 'pandapipes_shortest_f',
                             path=save_path, ext=save_gis_ext)
            save_gis_generic(gdf_shortest_r, 'pandapipes_shortest_r',
                             path=save_path, ext=save_gis_ext)
